...
namespace wisp
{
	namespace encode
	{
		vmcore::VmError SetIntegerRegisterValueWithEncoding(vmcore::Vm* vm, RegisterInt& reg, IntegerValueType encoding, uint64 base = 0u);
		vmcore::VmError GetIntegerConstantValueWithEncoding(vmcore::Vm* vm, IntegerValue& value, IntegerValueType encoding);
		vmcore::VmError SetFPRegisterValueWithEncoding(vmcore::Vm* vm, RegisterFP& reg, FPValueType encoding);
